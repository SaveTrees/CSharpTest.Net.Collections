...
namespace CSharpTest.Net.IO
{
    ...
    /// </summary>
    public class TransactedCompoundFile : IDisposable,
        ITransactable
    {
        enum LoadFrom
        {
        ...
        public class Options : ICloneable
        {
            ...
            /// </summary>
            public LoadingRule LoadingRule { get { return _loadingRule; } set { _loadingRule = value; } }
        ...
        public TransactedCompoundFile(string filename)
            : this(new Options(filename)
                   {
                       CreateNew = !File.Exists(filename)
                   })
        {
        }
...
[System.Runtime.InteropServices.DllImport("kernel32.dll", ExactSpelling = true, SetLastError = true)]
        ...
        void FlushStream(Stream stream)
        {
            ...
            {
                var handle = (SafeFileHandle)fs.GetType().GetField("_handle", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(fs);
        ...
        public void Commit<T>(Action<T> stageCommit, T value)
        {
            ...
            lock (_sync)
            {
                ...
                finally
                {
                    ...
                    foreach (var ifree in _reservedBlocks)
                    {
        ...
        private bool LoadSections(Stream stream, LoadFrom from)
        {
            ...
            var lastIndex = (int) (hsize*BlocksPerSection) - 1;

            var freeHandles = new OrdinalList();
            freeHandles.Ceiling = lastIndex;

            var usedBlocks = new OrdinalList();
        ...
        {
            var n = new FileSection(_sections.Length, BlockSize);
            lock (_sync)
            {
            ...
            var grow = new FileSection[_sections.Length + 1];
            _sections.CopyTo(grow, 0);
            grow[_sections.Length] = n;

            var freeblocks = _freeBlocks.Clone();
            freeblocks.Ceiling = grow.Length*BlocksPerSection - 1;

            var freehandles = _freeHandles.Clone();
            freehandles.Ceiling = grow.Length*BlocksPerSection - 1;
            // First and last handles/blocks are reserved by the section
            var lastFree = grow.Length*BlocksPerSection - 1;
            var firstFree = lastFree - BlocksPerSection + 2;
            for (var i = firstFree; i < lastFree; i++)
            {
                freehandles.Add(i);
                freeblocks.Add(i);
            }
        ...
        private uint TakeBlocks(int blocksNeeded)
        {
            lock (_sync)
            {
                var rescan = false;
                var resized = false;
                var startingFrom = _prevFreeBlock;
                var endingBefore = int.MaxValue;
                while (true)
                {
                    var found = 0;
                    var last = int.MinValue;
                    var first = int.MaxValue;
                    foreach (var free in _freeBlocks.EnumerateRange(startingFrom, endingBefore))
                    {
                        ...
                        {
                            var start = free - (blocksNeeded - 1);
                            for (var i = start; i <= free; i++)
                                _freeBlocks.Remove(i);

                            var blockId = (uint) start;
        ...
        {
            var free = block.Section*BlocksPerSection + block.Offset;
            if (free > 0)
            {
                ...
                for (var i = 0; i < block.ActualBlocks; i++)
        ...
        public uint Create()
        {
            ...
            lock (_sync)
            {
                while (handle == 0)
                {
                    foreach (var i in _freeHandles.EnumerateFrom(_prevFreeHandle))
                    {
            ...
            var blockId = _sections[href.Section][href.Offset];
            if (blockId != 0)
                throw new InvalidDataException();
        ...
        public void Write(uint handle, byte[] bytes, int offset, int length)
        {
            ...
            var oldblockId = _sections[href.Section][href.Offset];

            var blocksNeeded = Math.Max(1, (length + BlockHeaderSize + BlockSize - 1)/BlockSize);
            if (blocksNeeded > BlocksPerSection - 2)
                throw new ArgumentOutOfRangeException("length");

            var blockId = TakeBlocks(blocksNeeded);
        ...
        struct HandleRef
        {
            ...
            {
                var blocksPerSection = blockSize >> 2;
        ...
        class FileSection
        {
            ...
            {
                var blockdata = new byte[BlockSize*block.ActualBlocks];
            ...
            public Stream Read(ref BlockRef block, bool headerOnly, FGet fget)
            {
                ...
                do
                {
                    retry = false;
                    var position = _sectionPosition + BlockSize*block.Offset;
                    bytes = new byte[headerOnly
                        ? BlockHeaderSize
                        : block.ActualBlocks*BlockSize];
                    ...
                    var blockId = GetUInt32(bytes, OffsetOfBlockId);

                    if (headerSize < BlockHeaderSize || blockId != block.Identity || (block.Count < 16 && block.ActualBlocks != block.Count) || (block.Count == 16 && block.ActualBlocks < 16))
                        throw new InvalidDataException();
            ...
            {
                var baseHandle = unchecked(BlocksPerSection*_sectionIndex);
                //reserved: first and last block
                usedBlocks.Add(baseHandle);
                usedBlocks.Add(baseHandle + BlocksPerSection - 1);

                for (var handle = 1; handle < BlocksPerSection - 1; handle++)
                {
                    var data = ReadUInt32(handle);
                    if (data == 0)
                        freeHandles.Add(baseHandle + handle);
                    else
                    {
                        var block = new BlockRef(data, BlockSize);
                        var blockId = (int) block.Identity & 0x0FFFFFFF;

                        if (block.Count == 16)
                        {
                            var position = (long) BlocksPerSection*BlockSize*block.Section;
            ...
            {
                var crc1 = GetUInt32(_blockData, 0);
                var crc2 = GetUInt32(_blockData, _blockData.Length - 4);

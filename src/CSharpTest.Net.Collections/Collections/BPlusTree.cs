...
namespace CSharpTest.Net.Collections
{
	...
	/// </summary>
	public partial class BPlusTree<TKey, TValue> : IDisposable,
		ITransactable,
		...
		IConcurrentDictionary<TKey, TValue>
	{
		...
		public BPlusTree(BPlusTreeOptions<TKey, TValue> ioptions)
		{
			...
			switch (_options.CachePolicy)
			{
				...
				case CachePolicy.None:
					...
					break;
				default:
					throw new InvalidConfigurationValueException("CachePolicy");
			}
			...
			if (_options.LogFile != null && !_options.ReadOnly)
			{
				...
				{
					var commit = _options.ExistingLogAction == ExistingLogAction.ReplayAndCommit ||
								 (_options.ExistingLogAction == ExistingLogAction.Default && fileExists);
		...
		{
			NotDisposed();
			var locked = requiresLock && _selfLock.TryWrite(LockTimeout);
			try
			{
				if (_storage.Storage is INodeStoreWithCount)
				{
					((INodeStoreWithCount) _storage.Storage).Count = _hasCount
						? _count
						: -1;
				}
		...
		public void EnableCount()
		{
			if (_hasCount)
				return;
			_count = 0;
			using (var root = LockRoot(LockType.Read, "EnableCount", true))
		...
		public TValue this[TKey key]
		{
			...
			set
			{
				var ii = new InsertValue<TKey, TValue>(value, true);
				AddEntry(key, ref ii);
			}
		}
		...
		struct RootLock : IDisposable
		{
			...
			public RootLock(BPlusTree<TKey, TValue> tree, LockType type, bool exclusiveTreeAccess, string methodName)
			{
				...
				_tree = tree;
				_type = type;
				_version = type == LockType.Read
					? tree._storage.CurrentVersion
					: null;
		...
		{
			var ui = new UpdateInfo<TKey, TValue>(value);
			bool updated;
			using (var root = LockRoot(LockType.Update, "Update"))
			{
				updated = Update(root.Pin, key, ref ui);
			}
			DebugComplete("Updated({0}) = {1}", key, updated);
		...
		{
			var ui = new UpdateIfValue<TKey, TValue>(value, comparisonValue);
			bool updateAllowed;
			using (var root = LockRoot(LockType.Update, "Update"))
			{
				updateAllowed = Update(root.Pin, key, ref ui);
			}
			DebugComplete("Updated({0}) = {1}", key, updateAllowed);
		...
		{
			var ui = new UpdateInfo<TKey, TValue>(fnUpdate);
			bool result;
			using (var root = LockRoot(LockType.Update, "Update"))
			{
		...
		{
			var ii = new InsertValue<TKey, TValue>(item.Value, false);
		...
		{
			var ii = new InsertValue<TKey, TValue>(value, false);
			AddEntry(key, ref ii);
		}
		...
		{
			var ii = new FetchValue<TKey, TValue>(value);
			AddEntry(key, ref ii);
			return ii.Value;
		}
		...
		{
			var ii = new InsertionInfo<TKey, TValue>(fnCreate, IgnoreUpdate);
			AddEntry(key, ref ii);
			return ii.Value;
		}
		...
		{
			var ii = new InsertionInfo<TKey, TValue>(addValue, fnUpdate);
			AddEntry(key, ref ii);
			return ii.Value;
		}
		...
		{
			var ii = new InsertionInfo<TKey, TValue>(fnCreate, fnUpdate);
			AddEntry(key, ref ii);
			return ii.Value;
		}
		...
		{
			InsertResult result;
			using (var root = LockRoot(LockType.Insert, "AddOrUpdate"))
		...
		{
			var ri = new RemoveIfValue<TKey, TValue>(item.Key, item.Value);
		...
		{
			var ri = new RemoveAlways<TKey, TValue>();
		...
		{
			var ri = new RemoveIfPredicate<TKey, TValue>(fnCondition);
			return RemoveEntry(key, ref ri) == RemoveResult.Removed;
		}
		...
		{
			RemoveResult result;
			using (var root = LockRoot(LockType.Delete, "Remove"))
		...
		public ICollection<TKey> Keys
		{
			get
			{
				return _keysCollection != null
					? _keysCollection
					: (_keysCollection = new KeyCollection(this));
			}
		}
		...
		/// </summary>
		public bool IsReadOnly { get { return _options.ReadOnly; } }
